...
Customising module behaviour often requires touching multiple files deep
inside the crate, not just `src/config.rs`.  For example, to change how
workspace buttons are styled you must edit `src/modules/workspace/i3.rs`
and potentially `src/modules/mod.rs` (the `AppState` struct).  There is
no plugin system or declarative styling layer — you are expected to edit
Rust source directly.

### Window-manager support

Only i3 (and Sway, over the same IPC protocol) is supported.  There is no
adapter for other window managers or desktop environments (bspwm, dwm,
Hyprland, KDE, GNOME, …).  Adding a new WM requires writing a new IPC
backend and hooking it into `src/modules/workspace/mod.rs`.
